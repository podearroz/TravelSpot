...
class SupabaseService {
  ...
  static Future<List<Place>> fetchPlaces({String? type, String? cuisine}) async {
    try {
      var query = _client.from('places').select();
      if (type != null && type.isNotEmpty) query = query.eq('type', type);
      if (cuisine != null && cuisine.isNotEmpty) query = query.eq('cuisine', cuisine);
  ...
  static Future<List<Review>> fetchReviews(String placeId) async {
    try {
      final res = await _client.from('reviews').select().eq('place_id', placeId).execute();
      final data = (res.data as List).cast<Map<String, dynamic>>();
  ...
  static Future<bool> editReview(String id, Map<String, dynamic> changes) async {
    try {
